...
export const mockOrders: Order[] = [
  ...
  { id: 'o2', kode_order: 'LD-20250414-002', customer_nama: 'Eko Prasetyo', customer_hp: '08198765432', mitra_id: 'm1', jenis: 'satuan', berat: 3, harga: 45000, status: 'selesai_pusat', catatan: '', tanggal_masuk: today, tanggal_selesai: null },
  { id: 'o3', kode_order: 'LD-20250413-001', customer_nama: 'Dewi Lestari', customer_hp: '08567891234', mitra_id: 'm2', jenis: 'kiloan', berat: 8, harga: 80000, status: 'selesai_closed', catatan: 'Express', tanggal_masuk: yesterday, tanggal_selesai: today },
  ...
  { id: 'o6', kode_order: 'LD-20250412-001', customer_nama: 'Hendra Wijaya', customer_hp: '08456789123', mitra_id: 'm2', jenis: 'kiloan', berat: 7, harga: 70000, status: 'selesai_closed', catatan: '', tanggal_masuk: '2025-04-12', tanggal_selesai: '2025-04-13' },
];
...
const generateStatusHistoryForOrder = (order: Order): OrderStatusLog[] => {
  ...
  const historyToGenerate = allStatuses.slice(0, currentStatusIndex + 1);
  const lastTimestamp = new Date(order.tanggal_masuk);

  historyToGenerate.forEach((status, index) => {
    // Add random time offset to make it look realistic
    const offsetHours = Math.random() * 3 + 1; // 1 to 4 hours
    lastTimestamp.setHours(lastTimestamp.getHours() + offsetHours);

    logs.push({
      id: `sl_${order.id}_${index}`,
      order_id: order.id,
      status: status,
...
export const mockStatusLogs: OrderStatusLog[] = mockOrders.flatMap(generateStatusHistoryForOrder);
